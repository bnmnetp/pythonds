...
from trees.binary_search_tree import BinarySearchTree, BinaryTreeNode
    ...
    def _put(self, key, value, current_node):
        ...
        if key < current_node.key:
            if current_node.get_child_left():
                ...
                current_node.child_left = AVLTreeNode(key, value, 0, parent=current_node)
                self.update_balance(current_node.child_left)
        else:
            if current_node.get_child_right():
                self._put(key, value, current_node.child_right)
            else:
                current_node.child_right = AVLTreeNode(key, value, 0, parent=current_node)
